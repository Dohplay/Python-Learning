...
def convolution_filter(img, kernelTable,kernel):
...
def test_convolution_filter(name):
    img = load_image(choose_file())
    new_image = convolution_filter(img, kernels, kernels[name])
    show(new_image)
    # save_as(new_image, "convolution_blur.jpg")    
    

def build_kernal_table():
    kernalDict = {
    'blur':(
        (1, 1, 1),
        (1, 1, 1),
